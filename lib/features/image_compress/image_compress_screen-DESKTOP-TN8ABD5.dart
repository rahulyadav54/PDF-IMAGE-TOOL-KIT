...
class _ImageCompressScreenState extends ConsumerState<ImageCompressScreen> {
  ...
  Future<void> _compress() async {
    ...
    try {
      ...
      await Navigator.of(context).push<void>(
        MaterialPageRoute(
          builder: (context) => ResultScreen(
            ...
            conversionLabel: '${result.format.label} at ${result.quality}% quality',
            onProcessAnother: () {
      ...
      if (mounted) {
        setState(() => _fatalError = 'Unable to compress this image. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_fatalError != null) {
      return Scaffold(
        appBar: AppBar(title: const ToolAppBarTitle(tool: ToolType.imageCompress)),
        body: ErrorView(
          message: _fatalError!,
    ...
    return Stack(
      children: [
              ...
              children: [
                ...
                if (_selectedPath == null) ...[
                  ...
                  SizeComparisonCard(
                    ...
                    showEstimate: true,
                    estimateSubtitle: 'Approximate — actual size depends on image content',
                  ),
                  const SizedBox(height: 24),
                  FilledButton.icon(
        ...
        if (_isCompressing) const LoadingOverlay(message: 'Compressing image...'),
      ],
    );
  }
}
