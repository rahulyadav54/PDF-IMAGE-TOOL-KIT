...
class CompressPdfScreen extends ConsumerStatefulWidget {
  const CompressPdfScreen({super.key, this.initialPath});
...
class _CompressPdfScreenState extends ConsumerState<CompressPdfScreen> {
  CompressionLevel get _effectiveLevel =>
      _level ?? ref.read(appPreferencesProvider).compressionLevel;

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && _level == null) {
        setState(() => _level = ref.read(appPreferencesProvider).compressionLevel);
      }
    });
    ...
    if (initialPath != null && initialPath.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          _loadFile(() => Future<String?>.value(initialPath));
        }
      });
    }
  }
  ...
  Future<void> _compress() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() =>
            _fatalError = 'Unable to compress this PDF. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_fatalError != null) {
      return Scaffold(
        appBar:
            AppBar(title: const ToolAppBarTitle(tool: ToolType.compressPdf)),
        body: ErrorView(
          message: _fatalError!,
