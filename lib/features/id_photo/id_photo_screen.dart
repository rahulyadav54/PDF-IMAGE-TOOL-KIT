...
class IdPhotoScreen extends ConsumerStatefulWidget {
  const IdPhotoScreen({super.key, this.initialPath});
...
class _IdPhotoScreenState extends ConsumerState<IdPhotoScreen> {
  ...
  void initState() {
    ...
    if (initialPath != null && initialPath.isNotEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) _loadImage(() async => initialPath);
      });
    }
  }
  ...
  Future<void> _takePhoto() async {
    await _loadImage(() async {
      final path =
          await ref.read(documentScannerServiceProvider).scanSinglePage();
      return path;
    });
  }
      ...
      await Navigator.of(context).push<void>(
        MaterialPageRoute(
          builder: (context) => ResultScreen(
            ...
            conversionLabel:
                '${result.preset.name} • ${result.width}×${result.height} px',
            onProcessAnother: () {
        ...
        Scaffold(
          ...
          body: SafeArea(
            child: _selectedPath == null
                ...
                : _buildEditStep(context, scheme),
          ),
        ),
        if (_isLoading) const LoadingOverlay(message: 'Loading photo...'),
        if (_isProcessing)
          const LoadingOverlay(message: 'Creating ID photo...'),
      ],
    );
  }
          ...
          Container(
            ...
            child: const Center(
              child: Icon(Icons.badge_outlined,
                  size: 56, color: AppColors.electricBlue),
            ),
          ),
          const Spacer(),
          FilledButton.icon(
      ...
      children: [
        ...
        Slider(
          ...
          onChanged: _isProcessing
              ? null
              : (value) =>
                  _updateOptions(_options.copyWith(verticalFocus: value)),
        ),
        const SizedBox(height: AppSpacing.lg),
        FilledButton.icon(
