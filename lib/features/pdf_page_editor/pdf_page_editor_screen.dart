...
class PdfPageEditorScreen extends ConsumerStatefulWidget {
  const PdfPageEditorScreen({super.key, this.initialPath});

  final String? initialPath;

  @override
  ConsumerState<PdfPageEditorScreen> createState() =>
      _PdfPageEditorScreenState();
}
...
class _PdfPageEditorScreenState extends ConsumerState<PdfPageEditorScreen> {
  ...
  Future<void> _loadFile(Future<String?> Function() load) async {
    ...
    try {
      ...
      final pages = ref
          .read(pdfPageEditorServiceProvider)
          .createInitialPages(validation.pageCount);
  ...
  Future<void> _export() async {
    ...
    try {
      ...
      if (mounted) {
        setState(
            () => _fatalError = 'Unable to update this PDF. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_fatalError != null) {
      return Scaffold(
        appBar:
            AppBar(title: const ToolAppBarTitle(tool: ToolType.pdfPageEditor)),
        body: ErrorView(
          message: _fatalError!,
    ...
    return Stack(
      children: [
        Scaffold(
          appBar: AppBar(
              title: const ToolAppBarTitle(tool: ToolType.pdfPageEditor)),
          body: SafeArea(
            child: ListView(
                ...
                if (_selectedPath == null) ...[
                  ...
                  ReorderableListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Card(
                        key: ValueKey(
                            '${page.originalIndex}_${index}_${page.rotationSteps}'),
