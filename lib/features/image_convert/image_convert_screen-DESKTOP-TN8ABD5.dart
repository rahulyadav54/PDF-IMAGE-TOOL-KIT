...
class _ImageConvertScreenState extends ConsumerState<ImageConvertScreen> {
  ...
  Future<void> _convert() async {
    if (_selectedPath == null || _validation == null || _sourceFormat == null) return;

    if (_sourceFormat == _targetFormat) {
      setState(() => _fatalError = 'Choose a different output format to convert.');
      return;
    }
    ...
    try {
      ...
      await Navigator.of(context).push<void>(
        MaterialPageRoute(
          builder: (context) => ResultScreen(
            ...
            conversionLabel: '${result.sourceFormat.label} → ${result.targetFormat.label}',
            onProcessAnother: () {
      ...
      if (mounted) {
        setState(() => _fatalError = 'Unable to convert this image. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_fatalError != null) {
      return Scaffold(
        appBar: AppBar(title: const ToolAppBarTitle(tool: ToolType.imageConvert)),
        body: ErrorView(
          message: _fatalError!,
