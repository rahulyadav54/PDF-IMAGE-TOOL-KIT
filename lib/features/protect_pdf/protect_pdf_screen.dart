...
class ProtectPdfScreen extends ConsumerStatefulWidget {
  const ProtectPdfScreen({super.key, this.initialPath});
...
class _ProtectPdfScreenState extends ConsumerState<ProtectPdfScreen> {
  ...
  Future<void> _protect() async {
    ...
    try {
      ...
      if (mounted) {
        setState(
            () => _fatalError = 'Unable to lock this PDF. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Scaffold(
          appBar:
              AppBar(title: const ToolAppBarTitle(tool: ToolType.protectPdf)),
          body: SafeArea(
            child: ListView(
