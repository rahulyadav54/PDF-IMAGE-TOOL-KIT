...
class PdfToImageScreen extends ConsumerStatefulWidget {
  const PdfToImageScreen({super.key, this.initialPath});
...
class _PdfToImageScreenState extends ConsumerState<PdfToImageScreen> {
  ImageExportFormat get _effectiveFormat =>
      _format ?? ref.read(appPreferencesProvider).exportFormat;
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted && _format == null) {
        setState(() => _format = ref.read(appPreferencesProvider).exportFormat);
      }
    });
  ...
  Future<void> _convert() async {
    ...
    try {
        ...
        await Navigator.of(context).push<void>(
          MaterialPageRoute(
            builder: (context) => MultiFileResultScreen(
              ...
              subtitle:
                  '${result.pageCount} pages exported as ${_effectiveFormat.label}',
      ...
      if (mounted) {
        setState(
            () => _fatalError = 'Unable to export images. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    ...
    return Stack(
      children: [
        Scaffold(
          appBar:
              AppBar(title: const ToolAppBarTitle(tool: ToolType.pdfToImage)),
          body: SafeArea(
            child: ListView(
