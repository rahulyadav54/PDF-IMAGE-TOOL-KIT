...
class ImageStitchScreen extends ConsumerStatefulWidget {
  const ImageStitchScreen({super.key});
...
class _ImageStitchScreenState extends ConsumerState<ImageStitchScreen> {
  ...
  Future<void> _pickImages() async {
    ...
    try {
      ...
      setState(() => _fatalError = 'Unable to select images. Please try again.');
    } finally {
  ...
  Future<void> _stitch() async {
    ...
    try {
      ...
      if (mounted) {
        setState(() => _fatalError = 'Unable to stitch images. Please try again.');
      }
    } finally {
  ...
  Widget build(BuildContext context) {
    if (_fatalError != null) {
      return Scaffold(
        appBar: AppBar(title: const ToolAppBarTitle(tool: ToolType.imageStitch)),
        body: ErrorView(
          message: _fatalError!,
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: IconButton(
                          ...
                          onPressed: _isProcessing ? null : () => _removeAt(index),
                        ),
                      ),
                    );
                  }),
